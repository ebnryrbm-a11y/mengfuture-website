...
import { mockDb, type TeamMemberCMS } from '@/lib/mock-data'
...
export default function AdminTeamPage() {
  ...
  useEffect(() => {
    const data = mockDb.teamMembers.findMany().sort((a, b) => a.sortOrder - b.sortOrder)
    setTeam(data)
  }, [])
  ...
  const handleDelete = (id: number) => {
    if (confirm('确定要删除这个团队成员吗？')) {
      mockDb.teamMembers.delete(id)
      setTeam(mockDb.teamMembers.findMany().sort((a, b) => a.sortOrder - b.sortOrder))
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-dark-900">团队管理</h2>
          <p className="text-dark-500 mt-1">管理网站展示的团队成员</p>
        </div>
        <Link
          href="/admin/team/new"
      ...
      <div className="bg-white rounded-2xl border border-dark-200 overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-dark-50">
              <tr>
                ...
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-600">成员</th>
                <th className="px-6 py-4 text-left text-sm font-semibold text-dark-600">职位</th>
        ...
        {filteredTeam.length === 0 && (
          <div className="p-12 text-center">
            <p className="text-dark-500">暂无团队成员数据</p>
          </div>
        )}
      </div>
    </div>
